...
The current MCP handler already uses `responseMode: "auto"`, so quiet calls return JSON while a request can upgrade to SSE when it emits related messages. It still sets `legacy: "reject"`, exercises only the `2026-07-28` request envelope, and has no application-level MCP body bound. The groceries work will enable the stateless 2025 compatibility lane, add a parse-once 64 KiB ingress limit after Host/Origin validation, and cover both protocol eras plus the stable error boundaries. It will not introduce an MCP session or deprecated standalone SSE transport. [Adapter source](../../packages/mcp-adapter/src/index.ts), [test source](../../test/worker.test.ts), [response-mode semantics](https://github.com/modelcontextprotocol/typescript-sdk/blob/cc4b41617ce3601b1290d67216ea0b194a3cd9ac/packages/server/src/server/createMcpHandler.ts#L161-L180)
...
MCP remains stateless even when a domain tool owns durable application state. Groceries may expose a random bearer `listKey` in its schemas and store exactly one anonymous shopping-list document in a Durable Object. Possession authorizes that document without an account, profile or global identity; the key is not an MCP session ID. It appears only in tool arguments and successful results and may be retained by third-party clients under their own policies. Stamppot never places it in URLs, R2, logs, analytics or errors. Cloudflare recommends Durable Objects, D1, KV or R2 for cross-request application data rather than an MCP session ID. [Source](https://developers.cloudflare.com/agents/model-context-protocol/apis/handler-api/)
