...
Stamppot is a collection of small, dependable authless MCP servers for Dutch data. Contributions should keep the public interface small (and cheap), the data provenance explicit, and hosted use safe without an account.

## Changesets

Every pull request or direct tinkering on main must add a changeset. Use `pnpm changeset` and select every affected package with the appropriate semantic version bump. For documentation, tests, CI, or other changes that do not release a package, use `pnpm changeset --empty`. Write the summary for users of the changed behavior. The Changesets-generated release pull request is the only exception.
